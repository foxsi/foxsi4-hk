...
if __name__ == "__main__":
    print("\nconfiguring...")
    socket.sendto(set_power_health_setup, remote_endpoint)
    time.sleep(0.1)
    socket.sendto(set_temp1_init, remote_endpoint)
    time.sleep(0.1)
    socket.sendto(set_temp2_init, remote_endpoint)
    time.sleep(0.1)
    socket.sendto(start_temp1_convert, remote_endpoint)
    time.sleep(0.1)
    ...
    while True:
        ...
        socket.sendto(read_temp1_all, remote_endpoint)
        ...
        socket.sendto(read_temp2_all, remote_endpoint)
        ready = select.select([socket], [], [], timeout)
        if ready[0]:
            (data, sender) = socket.recvfrom(1024)
            print(data.hex())
        time.sleep(intercommand)
        socket.sendto(read_power_health_all, remote_endpoint)
        ready = select.select([socket], [], [], timeout)
        if ready[0]:
            (data, sender) = socket.recvfrom(1024)
            print(data.hex())
